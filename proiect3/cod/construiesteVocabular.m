...
%     cuvinteVizuale - matrice k x 128, fiecare linie reprezinta un cuvant vizual (un centru al unui cluster)
  ...
  for i=1:numarImagini,
    ...
    % calculeaza descriptorul HOG si patch-ul corespunzator pt fiecare
    puncte = genereazaPuncteCaroiaj(img,nrPuncteX,nrPuncteY,margine); %am generat punctele
    [HOGimg, patchimg] = calculeazaHistogrameGradientiOrientati(img, puncte, dimensiuneCelula);
    
    
    % pune in descriptoriHOG si patchuri ce ai calculat
    descriptoriHOG(i*100-99:i*100,:) = HOGimg;
    patchuri(i*100-99:i*100,:) = patchimg;
